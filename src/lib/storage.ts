import { ARLCompany, ClientCompany, PilaRecord, FieldVisit, MedicalRecord, RUIIntermediaryProfile, WhatsAppMessage, UserProfile, AgencyProfile } from '../types';
import { INITIAL_ARLS, INITIAL_CLIENTS, INITIAL_PILA_RECORDS, INITIAL_FIELD_VISITS, INITIAL_MEDICAL_RECORDS, INITIAL_RUI_PROFILE, INITIAL_WHATSAPP_MESSAGES, INITIAL_AGENCY_PROFILE, INITIAL_USER_PROFILES } from './data';
  ...
  try {
    ...
    const cleaned = parsed
      ...
      .map((a) => {
        ...
        return {
          ...a,
          economicActivityCommissions: a.economicActivityCommissions && a.economicActivityCommissions.length > 0
  ...
  try {
    ...
    return list.map((c) => {
      ...
      return {
        ...
        standardsCount: c.standardsCount || init?.standardsCount || (c.employeeCount > 50 || c.riskClass === 'CLASE_IV' || c.riskClass === 'CLASE_V' ? 60 : c.employeeCount >= 11 ? 21 : 7),
...
export const getStoredPilaRecords = (): PilaRecord[] => {
  ...
  try {
    ...
    return list.map((r) => {
      ...
      const rete = r.retefuenteAmount ?? (gross * 0.10);
      ...
      return {
        ...r,
        retefuenteRate: r.retefuenteRate ?? 0.10,
...
export const getStoredFieldVisits = (): FieldVisit[] => {
  ...
  try {
    ...
    return list.map((v) => ({
      ...
      costNotes: v.costNotes ?? (v.hasCost ? `Valorizado en $${v.visitCost} COP` : 'Cubierto 100% por retorno / intermediación ARL'),
    }));
  } catch (e) {
    return INITIAL_FIELD_VISITS;
  }
...
const LEADS_KEY = 'wappy_leads_v1';
